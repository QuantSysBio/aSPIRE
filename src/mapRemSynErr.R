...
if (length(errors) > 0) {
  ...
  if (length(ErrPeps) > 0) {
    ...
    plotTbl = DB[gsub("I","L",DB$pepSeq) %in% ErrPeps,] %>%
      group_by(pepSeq,biological_replicate,digestTime) %>%
      arrange(digestTime, .by_group = T) %>%
      mutate(technical_replicate = row_number())
    ...
    tmp = plotTbl %>%
      ...
      summarise(l = length(unique(technical_replicate)))
    
    if (length(unique(tmp$l)) == 0) {
      plotKinetics(plotTbl, outfile = paste0("results/",protein_name,"/plots/rawIntensities_SynErr.pdf"),
